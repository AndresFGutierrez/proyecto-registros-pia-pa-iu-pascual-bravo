...
<div class="max-w-2xl mx-auto py-10 sm:px-6 lg:px-8">
    <div class="bg-white shadow rounded-lg p-6">
        <h2 class="text-xl font-bold mb-4">Nueva Evaluación de Proyecto</h2>
            ...
            <div class="mb-4">
                ...
                <select name="id_evaluacion" id="id_evaluacion"
                    ...
                    @foreach($evaluaciones as $evaluacion)
                        <option value="{{ $evaluacion->id_evaluacion }}" {{ old('id_evaluacion') == $evaluacion->id_evaluacion ? 'selected' : '' }}>
                            {{ $evaluacion->fecha }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="id_evaluador" id="id_evaluador"
                    ...
                    @foreach($evaluadores as $evaluador)
                        <option value="{{ $evaluador->id_evaluador }}" {{ old('id_evaluador') == $evaluador->id_evaluador ? 'selected' : '' }}>
                            {{ $evaluador->nombre }}
                        </option>
                    @endforeach
                </select>
